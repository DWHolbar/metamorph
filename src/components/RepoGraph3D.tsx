...
function detectCapabilities(): DeviceCapabilities {
  ...
  return {
    sphereSegments: lowEnd ? 8 : 20,
    starsCount: lowEnd ? 800 : 3000,
...
function RepoNode({ repo, position, baseColor, visited, highlighted, sphereSegments }: NodeProps) {
    ...
    <group position={position}>
      ...
      {hovered && (
        <Html distanceFactor={80} center>
          <div className="bg-gray-900/95 dark:bg-zinc-950/95 border border-gray-700 dark:border-zinc-700 rounded-xl px-3 py-2.5 shadow-xl min-w-[180px] max-w-[240px]">
            ...
            <div className="flex items-center gap-2 mt-1.5 text-xs text-gray-500 pointer-events-none">
            ...
            <div className="flex items-center gap-2 mt-2 pt-2 border-t border-gray-700/50">
              <a
                ...
                className="flex-1 text-center text-[10px] px-2 py-1 rounded bg-blue-600/80 hover:bg-blue-500 text-white transition-colors"
              >
                Generate content →
              </a>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-[10px] px-2 py-1 rounded bg-gray-700/80 hover:bg-gray-600 text-gray-200 transition-colors"
              >
                GitHub
              </a>
            </div>
...
// ── Org cluster ───────────────────────────────────────────────────────────────
function OrgCluster({ repos, center, color, visited, highlighted, sphereSegments }: {
  repos: Repo[];
...
function Scene({ repos, visited, highlighted, flyTarget, caps }: {
  ...
  return (
    <>
      ...
      <Stars radius={600} depth={80} count={caps.starsCount} factor={5} fade />
...
export default function RepoGraph3D({ showInsights, onInsightsDismiss }: {
  ...
  return (
    <div className="relative w-full h-screen bg-gray-950">
      <Canvas camera={{ position: [0, 80, 450], fov: 55 }} dpr={caps.dpr}>
...
function PerformanceInsightsOverlay({ repos, onDismiss }: { repos: Repo[]; onDismiss?: () => void }) {
  ...
  const ORG_COLORS: Record<string, string> = {
    trailofbits: 'text-blue-400',
    crytic: 'text-purple-400',
    'lifting-bits': 'text-orange-400',
  };

  return (
    <div className="absolute top-4 right-4 w-72 bg-gray-900/95 border border-gray-700 rounded-xl p-4 shadow-2xl text-sm z-40">
      ...
      <button
        onClick={handleAskAI}
        className="w-full text-xs px-3 py-1.5 rounded-lg bg-blue-600/80 hover:bg-blue-500 text-white transition-colors"
      >
